...
test('filterRows select filter accepts single values and multi-select arrays', () => {
  const unfilteredRows = [
    row({ id: 1, group: 'A' }),
    row({ id: 2, group: 'B' }),
    row({ id: 3, group: 'C' }),
  ...
  assert.deepEqual(applyFilters(unfilteredRows, { group: { type: 'select', value: 'B' } }), [2]);
  assert.deepEqual(applyFilters(unfilteredRows, { group: { type: 'select', value: ['A', 'C'] } }), [1, 3]);
  // An empty selection means "no filter" for non-null values.
  assert.deepEqual(applyFilters(unfilteredRows, { group: { type: 'select', value: [] } }), [1, 2, 3]);
});

test('filterRows date-tree filter matches YYYY-MM buckets without timezone shifts', () => {
  const unfilteredRows = [
    row({ id: 1, createdAt: '2026-07-01T00:00:00' }),
    row({ id: 2, createdAt: '2026-06-30T23:59:59' }),
    row({ id: 3, createdAt: '2025-07-15' }),
    row({ id: 4, createdAt: null }),
    row({ id: 5, createdAt: 'not a date' }),
  ];

  assert.deepEqual(applyFilters(unfilteredRows, { createdAt: { type: 'date-tree', selected: ['2026-07'] } }), [1]);
  assert.deepEqual(applyFilters(unfilteredRows, { createdAt: { type: 'date-tree', selected: ['2026-06', '2025-07'] } }), [2, 3]);
  // Empty selection means "no filter", including null cells.
  assert.deepEqual(applyFilters(unfilteredRows, { createdAt: { type: 'date-tree', selected: [] } }), [1, 2, 3, 4, 5]);
});

test('filterRows scopes the global filter to globalFilterFields when provided', () => {
  const unfilteredRows = [
    row({ id: 1, name: 'needle', group: 'hay' }),
    row({ id: 2, name: 'hay', group: 'needle' }),
  ];
  ...
  assert.deepEqual(rowIds(scoped), [1]);

  const unscoped = filterRows({
    rows: unfilteredRows,
    columns: columnDefs,
    visibleColumns: allVisibleColumns,
    globalFilter: 'needle',
